...
def find_antinodes(loc1: Tuple[int, int], loc2: Tuple[int, int],
                   rows: int, cols: int,
                   unlimited=False) -> List[Tuple[int, int]]:
    ...
    while (antinode1[0] >= 0 and antinode1[0] < rows and
            antinode1[1] >= 0 and antinode1[1] < cols):
    ...
    while (antinode2[0] >= 0 and antinode2[0] < rows and
            antinode2[1] >= 0 and antinode2[1] < cols):
    ...
    for antenna, locs in attenna_locs.items():
        for i in range(len(locs)):
            for j in range(i + 1, len(locs)):
                antinodes_part_1.update(
                    find_antinodes(locs[i], locs[j], rows, cols))
